...
Customer-owned Software Assurance core counts and optional customer-specific
annualized License+SA prices are supplied through portable Power Query
parameters and are deliberately separated from Arc billing metadata. The
dedicated `Licensing Position` page compares those trusted inputs with forecast
Azure Hybrid Benefit core demand and the assessment's explicit SQL PAYG licence
component. Follow the [licensing input setup](LICENSING-INPUTS.md) to configure
the edition-covered cores and optional pricing.
